...
(defvar-local acm-terminal-hover-doc nil
  "Popon frame for standalone hover documentation.")

(defvar-local acm-terminal-hover-doc-content nil
  "Content of hover documentation.")
...
(defun acm-terminal-popup-documentation--callback (value)
  ...
  (when (and (stringp value) (not (string-empty-p value)))
    ...
    ;; Create popon frame if it doesn't exist
    (unless (popon-live-p acm-terminal-hover-doc)
      (let ((pos (popon-x-y-at-pos (point))))
        (setq acm-terminal-hover-doc 
              (acm-terminal-make-popon (cons "" 0) pos))))
    ...
    (pcase-let* ((`(,edge-left ,edge-top ,edge-right ,edge-bottom) (window-inside-edges))
                 (textarea-width (- (window-width)
                                   (+ (- edge-left (window-left-column))
                                      (acm-terminal-line-number-display-width))))
                 (textarea-height (- edge-bottom edge-top))
                 (`(,cursor-x . ,cursor-y) (popon-x-y-at-pos (point)))
                 (doc-max-width (min acm-terminal-doc-max-width (- textarea-width 2)))
                 (doc-lines (acm-terminal-doc-render value (1- doc-max-width)))
                 (doc-max-height (max 10 (/ textarea-height 3)))
                 (doc-display-lines (seq-take doc-lines doc-max-height))
                 (doc-w (1+ (1- doc-max-width)))
                 (doc-h (length doc-display-lines))
                 (bottom-free-h (- textarea-height cursor-y 1)))
      
      ;; Set popon properties
      (plist-put (cdr acm-terminal-hover-doc) :width doc-w)
      (plist-put (cdr acm-terminal-hover-doc) :lines doc-display-lines)
      
      ;; Position: prefer below cursor, but move above if not enough space
      (if (>= bottom-free-h doc-h)
          ;; Display below cursor
          (progn
            (plist-put (cdr acm-terminal-hover-doc) :x cursor-x)
            (plist-put (cdr acm-terminal-hover-doc) :y (1+ cursor-y)))
        ;; Display above cursor
        (plist-put (cdr acm-terminal-hover-doc) :x cursor-x)
        (plist-put (cdr acm-terminal-hover-doc) :y (max 0 (- cursor-y doc-h))))
...
(defun acm-terminal-popup-documentation-scroll-up (&optional arg)
  ...
  (interactive)
  (when (and (popon-live-p acm-terminal-hover-doc) 
             acm-terminal-hover-doc-content)
    (pcase-let*
        ((`(,doc-x . ,doc-y) (popon-position acm-terminal-hover-doc))
         (`(,doc-w . ,doc-h) (popon-size acm-terminal-hover-doc))
         (total-lines (acm-terminal-doc-render acm-terminal-hover-doc-content (1- doc-w)))
         (scroll-start (- (+ acm-terminal-hover-doc-scroll-start doc-h) 
                         (or next-screen-context-lines 2)))
         (taken-lines (when (< scroll-start (length total-lines))
                       (seq-take (nthcdr scroll-start total-lines) doc-h))))
      (when taken-lines
        (plist-put (cdr acm-terminal-hover-doc) :lines taken-lines)
        (popon-redisplay)
        (setq acm-terminal-hover-doc-scroll-start scroll-start)))))

(defun acm-terminal-popup-documentation-scroll-down (&optional arg)
  "Scroll hover documentation down."
  (interactive)
  (when (and (popon-live-p acm-terminal-hover-doc)
             acm-terminal-hover-doc-content)
    (pcase-let*
        ((`(,doc-x . ,doc-y) (popon-position acm-terminal-hover-doc))
         ...
         (scroll-start (+ (- acm-terminal-hover-doc-scroll-start doc-h)
                         (or next-screen-context-lines 2)))
         (taken-lines (when (<= 0 scroll-start)
                       (seq-take (nthcdr scroll-start total-lines) doc-h))))
      (when taken-lines
        (plist-put (cdr acm-terminal-hover-doc) :lines taken-lines)
        (popon-redisplay)
...
(defvar acm-terminal-advices
  '((acm-frame-init-colors :override acm-terminal-init-colors)
    ...
    (lsp-bridge-popup-documentation--callback :override acm-terminal-popup-documentation--callback)
    (lsp-bridge-popup-documentation-scroll-up :override acm-terminal-popup-documentation-scroll-up)
    (lsp-bridge-popup-documentation-scroll-down :override acm-terminal-popup-documentation-scroll-down)
    (lsp-bridge-hide-doc-tooltip :override acm-terminal-hide-hover-doc))
